...
const handleSaveUrgensiSurat = () => {
  ...
  // Generate new ID
  const newId = Math.max(...allUrgensiSuratData.value.map(item => item.id)) + 1;
...
const handleSaveEditUrgensiSurat = () => {
  ...
  if (itemToEdit.value) {
    const index = allUrgensiSuratData.value.findIndex(p => p.id === itemToEdit.value.id);
    if (index > -1) {
      // Update the item
...
const handleConfirmDelete = () => {
  if (itemToDelete.value) {
    const index = allUrgensiSuratData.value.findIndex(p => p.id === itemToDelete.value.id);
    if (index > -1) {
  ...
  <div class="space-y-4">
    ...
    <div class="border-neutral-9 rounded-lg border bg-white p-6 shadow-sm">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-gray-title text-2xl font-bold">Daftar Urgensi Surat</h1>          
        </div>
        <UButton
          icon="ph:plus"
          size="lg"          
      ...
      <template #nama-data="{ row }">
        <div class="flex items-center gap-3">
          <div class="h-8 w-8 rounded-lg bg-primary-50 flex items-center justify-center">
            <UIcon name="ph:clock" class="h-4 w-4 text-primary-600" />
          </div>
